...
include '../lib_functions/makeDir.php';
class cls_image
{

    public $error_no    = 0;
    public $error_msg   = '';
    public $images_dir  = '/upload';
    public $bgcolor     = '';
     ...
     * @param       array       dir          文件要上传在$this->images_dir下以当月命名的目录下
    ...
    {   
        ...
        {
            /* 创建当月目录 */
            $dir = ROOT_PATH . $this->images_dir . '/' . $time_dir . '/';
        }
        else
        {
            /* 创建目录 */
            $dir = ROOT_PATH . $this->images_dir . '/' . $dir . '/'.$time_dir."/";
        ...
        if (!file_exists($dir))
        {
            ...
            {
                /* 创建目录失败 */
                $this->error_msg = sprintf($this->get_error_msg('directory_readonly'), $dir);
        ...
        { 
            $this->error_msg = $this->get_error_msg('invalid_upload_image_type');
            // $this->error_no  =  ERR_INVALID_IMAGE_TYPE;
            return false;
        }
        
		/* 允许上传的文件类型 */
        $allow_file_types = '|GIF|JPG|JPEG|PNG|BMP|SWF|mp4|';
        ...
        else
        {
            ...
            $this->error_msg = sprintf($this->get_error_msg('upload_failure'), $upload['name']);
     ...
     */
     public function make_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='',$max_num=0)
    {
         ...
         if ($gd == 0)
         {
             $this->error_msg = $this->get_error_msg('missing_gd');
             return false;
         }
        ...
        if (!$org_info)
        {
            $this->error_msg = sprintf($this->get_error_msg('missing_orgin_image'), $img);
        ...
        {
            $this->error_msg = sprintf($this->get_error_msg('nonsupport_type'), $this->type_maping[$org_info[2]]);
		...
		if($max_num>0){
			...
			if( $img_height>$img_width){
				if($img_height <= $max_num){
					$thumb_height = $img_height;
					$thumb_width = $img_width;
				}else{
					$thumb_height = $max_num;
					$sc = $img_height/$thumb_height;
					$thumb_width = $img_width /$sc;
				}
			}else{
				if($img_width<=$max_num){
					$thumb_height = $img_height;
					$thumb_width = $img_width;
				}else{
					...
					$sc = $img_width/$thumb_width;
					$thumb_height = $img_height /$sc;
				}
			}
		}
		...
		*/
        ...
        if (empty($path))
        {
            $dir = ROOT_PATH . $this->images_dir . '/' .$brand_name_en.date('Ym').'/';
        }
        else
        {
            $dir = $path.$brand_name_en;
        }
        ...
        {
            $this->error_msg = $this->get_error_msg('writting_failure');
            // $this->error_no   = ERR_DIRECTORY_READONLY;
     ...
     * @return  mix         如果成功返回缩略图的路径，失败则返回false
     */
     public function get_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='',$max_num=0)
    {
     ...
     public function add_watermark($filename, $target_file='', $watermark='', $watermark_place='', $watermark_alpha = 0.65)
    {
        ...
        {
            $this->error_msg  = sprintf($this->get_error_msg('missing_orgin_image'), $filename);
            // $this->error_no   = ERR_IMAGE_NOT_EXISTS;
        ...
        if (!$watermark_handle)
        {
            $this->error_msg = sprintf($this->get_error_msg('create_watermark_res'), $this->type_maping[$watermark_info[2]]);
        ...
        if (!$source_handle)
        {
            $this->error_msg = sprintf($this->get_error_msg('create_origin_image_res'), $this->type_maping[$source_info[2]]);
            // $this->error_no = ERR_INVALID_IMAGE;
        ...
        switch ($source_info[2] )
        {
            ...
            default:
                $this->error_msg = $this->get_error_msg('creating_failure');
                // $this->error_no = ERR_NO_GD;
     ...
     public function validate_image($path)
    {
        if (empty($path))
        {
            $this->error_msg = $this->get_error_msg('empty_watermark');
        ...
        if (!file_exists($path))
        {
            $this->error_msg = sprintf($this->get_error_msg('missing_watermark'), $path);
            // $this->error_no = ERR_IMAGE_NOT_EXISTS;
            return false;
        }
        ...
        if (!$image_info)
        {
            $this->error_msg = sprintf($this->get_error_msg('invalid_image_type'), $path);
            // $this->error_no = ERR_INVALID_IMAGE;
            return false;
        }
        ...
        {
            $this->error_msg = sprintf($this->get_error_msg('nonsupport_type'), $this->type_maping[$image_info[2]]);
            // $this->error_no  =  ERR_NO_GD;
            return false;
        }
    ...
    public function get_error_msg($msg) {
        $error_msg['directory_readonly'] = '目录 % 不存在或不可写';
        $error_msg['invalid_upload_image_type'] = '不是允许的图片格式';
        $error_msg['upload_failure'] = '文件 %s 上传失败。';
        $error_msg['missing_gd'] = '没有安装GD库';
        $error_msg['missing_orgin_image'] = '找不到原始图片 %s ';
        $error_msg['nonsupport_type'] = '不支持该图像格式 %s ';
        $error_msg['creating_failure'] = '创建图片失败';
        $error_msg['writting_failure'] = '图片写入失败';
        $error_msg['empty_watermark'] = '水印文件参数不能为空';
        $error_msg['missing_watermark'] = '找不到水印文件%s';
        $error_msg['create_watermark_res'] = '创建水印图片资源失败。水印图片类型为%s';
        $error_msg['create_origin_image_res'] = '创建原始图片资源失败，原始图片类型%s';
        $error_msg['invalid_image_type'] = '无法识别水印图片 %s ';
        $error_msg['file_unavailable'] = '文件 %s 不存在或不可读';
